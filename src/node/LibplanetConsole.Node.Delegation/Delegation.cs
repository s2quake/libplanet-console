...
internal sealed class Delegation(INode node, IBlockChain blockChain, IBank bank)
    : NodeContentBase(nameof(Delegation)), IDelegation
{
    ...
    public async Task StakeAsync(FungibleAssetValue ncg, CancellationToken cancellationToken)
    {
        var stakeAction = new StakeAction(ncg);
    ...
    {
        var setValidatorCommissionAction = new SetValidatorCommission(node.Address, commission);
