...
namespace LibplanetConsole.Nodes.Guild.Tracers;
...
internal sealed class GuildEventTracer : IApplicationService, IDisposable
{
    ...
    private readonly IGuildNode _guildNode;

    [ImportingConstructor]
    public GuildEventTracer(INode node, IBlockChain blockChain, IGuildNode guildNode)
    {
        _node = node;
        _blockChain = blockChain;
        _guildNode = guildNode;
        _actionHandlers = new()
        {
    ...
    private async void Node_BlockAppended(object? sender, BlockEventArgs e)
    {
        ...
        {
            if (ContainsAction(blockInfo, typeId, out var transactionInfo, out var actionInfo))
            {
                await handler(transactionInfo, actionInfo, default);
            }
        }
    }
        ...
        TransactionInfo transactionInfo, ActionInfo actionInfo, CancellationToken cancellationToken)
    {
        if (transactionInfo.Signer != _node.Address)
        {
            ...
            var guildAddress = await _guildNode.GetGuildAsync(address, cancellationToken);
        ...
        TransactionInfo transactionInfo, ActionInfo actionInfo, CancellationToken cancellationToken)
    {
        if (transactionInfo.Signer != _node.Address)
        {
            ...
            var address = transactionInfo.Signer;
            var guildMessage = await _guildNode.GetGuildAsync(height, address, cancellationToken);
        ...
        TransactionInfo transactionInfo, ActionInfo actionInfo, CancellationToken cancellationToken)
    {
        if (transactionInfo.Signer != _node.Address)
        {
            ...
            var action = await blockChain.GetActionAsync<ApplyGuild>(
                transactionInfo.Id, actionInfo.Index, default);
        ...
        TransactionInfo transactionInfo, ActionInfo actionInfo, CancellationToken cancellationToken)
    {
        if (transactionInfo.Signer != _node.Address)
        {
            ...
            var action = await blockChain.GetActionAsync<AcceptGuildApplication>(
                transactionInfo.Id, actionInfo.Index, default);
            var guildAddress = await _guildNode.GetGuildAsync(
